 ...
 *
 * 유니코드 정규화를 위한 클래스를 정의한다.
 * PHP Internationalization 모듈이 존재하지 않을 경우에만 해당 클래스가 정의된다.
 *
 * @file /classes/Normalizer.php
    ...
    class Normalizer
    {
        ...
        public static function normalize($s, $form = self::NFC)
        {
            ...
            if (
                null !==
                ($mbEncoding =
                    2 /* MB_OVERLOAD_STRING */ & (int) ini_get('mbstring.func_overload')
        ...
        private static function decompose($s, $c)
        {
            ...
            while ($i < $len) {
                ...
                if ($uchr < "\xEA\xB0\x80" || "\xED\x9E\xA3" < $uchr) {
                    ...
                    $uchr =
                        "\xE1\x84" . chr(0x80 + (int) ($j / 588)) . "\xE1\x85" . chr(0xa1 + (int) (($j % 588) / 28));
                    if ($j %= 28) {
