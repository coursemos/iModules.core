...
class Cache {
	...
	public static function script(string $group,string|int $path=null):array|string {
		...
		if ($path !== null) {
			...
			if (Configs::debug() == true) {
				...
				$refresh = false;
				$cached_time = is_file(Configs::cache().'/'.$group.'.js') === true ? filemtime(Configs::cache().'/'.$group.'.js') : 0;
				...
				if ($cached_time < time() - 300) {
					foreach (self::$_scripts[$group] as $path) {
						if (is_file(Configs::path().$path) == false) continue;
	...
	public static function scss(string $path):string {
		if (is_file(Configs::path().$path) == false) return $path;
		
		$cached_file = preg_replace('/^\./','',str_replace('/','.',$path));
		$cached_time = is_file(Configs::cache().'/'.$cached_file.'.css') === true ? filemtime(Configs::cache().'/'.$cached_file.'.css') : 0;
	...
	public static function style(string $group,string|int $path=null):array|string {
		...
		if ($path !== null) {
			...
			if (Configs::debug() == true) {
				...
				$refresh = false;
				$cached_time = is_file(Configs::cache().'/'.$group.'.css') === true ? filemtime(Configs::cache().'/'.$group.'.css') : 0;
