...
class Scroll {
     ...
     */
    static create($dom, scrollable = null) {
        if (scrollable === null) {
     ...
     */
    updatePosition() {
        if (this.latestTargetSize.width === this.getTargetSize('x') &&
    ...
    deactive(direction, delay = 0) {
        ...
        if (delay > 0) {
            $axis.setData('timer', setTimeout(($scrollbar) => {
                $scrollbar.removeClass('active');
            }, delay * 1000, $axis), false);
        }
        else {
        ...
        if (this.isScrollable(direction) == true) {
            ...
            switch (direction) {
                case 'x':
                    ...
                    break;
                case 'y':
                    $thumb.setStyle('height', Math.ceil(($track.getHeight() / this.$dom.getScrollHeight()) * 100) + '%');
